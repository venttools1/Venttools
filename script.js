...
function buildFDSiteSheet(){
  // Open the tab immediately from the user's click. Mobile browsers may block
  // a new tab if it is opened only after prompts or other work has completed.
  const w=window.open("about:blank","_blank");
  if(!w){fdMsg("warn","Allow pop-ups to open the site instruction sheet.");return;}
  try{
    w.document.open();
    w.document.write('<!doctype html><html><head><meta charset="utf-8"><title>Preparing site sheet…</title></head><body style="font-family:Arial,sans-serif;padding:24px">Preparing site instruction sheet…</body></html>');
    w.document.close();
  }catch(e){}
  ...
  const diagram=isCircle
    ? `<div class="opening-diagram"><svg viewBox="0 0 520 360" role="img" aria-label="VentTools schematic of circular finished opening"><rect x="18" y="18" width="484" height="324" rx="18" fill="#edf2f6" stroke="#607487" stroke-width="4"/><circle cx="260" cy="178" r="126" fill="#d8e7f2" stroke="#0b6fae" stroke-width="5"/><circle cx="260" cy="178" r="90" fill="#f4f7f9" stroke="#394b59" stroke-width="5"/><line x1="134" y1="325" x2="386" y2="325" stroke="#172033" stroke-width="3"/><path d="M134 318v14M386 318v14" stroke="#172033" stroke-width="3"/><text x="260" y="351" text-anchor="middle" font-size="18" font-weight="700">${esc(r.opening)}</text><text x="260" y="174" text-anchor="middle" font-size="18" font-weight="700">Nominal damper</text><text x="260" y="198" text-anchor="middle" font-size="16">${esc(r.damper)}</text><text x="260" y="44" text-anchor="middle" font-size="15" fill="#0b6fae" font-weight="700">Finished opening / structural preparation</text></svg></div>`
    : `<div class="opening-diagram"><svg viewBox="0 0 620 390" role="img" aria-label="VentTools schematic of rectangular finished opening"><rect x="28" y="28" width="564" height="324" rx="16" fill="#edf2f6" stroke="#607487" stroke-width="4"/><rect x="105" y="75" width="410" height="230" fill="#d8e7f2" stroke="#0b6fae" stroke-width="5"/><rect x="165" y="117" width="290" height="146" fill="#f4f7f9" stroke="#394b59" stroke-width="5"/><text x="310" y="185" text-anchor="middle" font-size="19" font-weight="700">Nominal damper</text><text x="310" y="211" text-anchor="middle" font-size="17">${esc(r.damper)}</text><text x="310" y="62" text-anchor="middle" font-size="16" fill="#0b6fae" font-weight="700">Finished opening / structural preparation</text><line x1="105" y1="340" x2="515" y2="340" stroke="#172033" stroke-width="3"/><path d="M105 332v16M515 332v16" stroke="#172033" stroke-width="3"/><text x="310" y="377" text-anchor="middle" font-size="18" font-weight="700">${esc(r.opening)}</text></svg></div>`;
  const html=`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(ref)} Site Instruction Sheet</title><style>body{font-family:Arial,sans-serif;color:#172033;margin:32px;line-height:1.45}h1{color:#064b82;margin-bottom:4px}.meta{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:20px 0}.card{border:2px solid #d8e1ea;border-radius:12px;padding:16px;margin:14px 0}.result{border-color:#75b9e8;background:#eef7ff;text-align:center;font-size:28px;font-weight:800}.label{font-size:12px;text-transform:uppercase;color:#5f6e7d;font-weight:700}.warning{background:#fff7df;border-color:#e4bd52}.opening-diagram{max-width:760px;margin:0 auto}.opening-diagram svg{display:block;width:100%;height:auto}.diagram-note{text-align:center;font-size:12px;color:#5f6e7d;margin-top:8px}ul{padding-left:22px}.print-btn{appearance:none;border:0;border-radius:10px;padding:12px 16px;background:#064b82;color:white;font-weight:700;font-size:16px}footer{margin-top:24px;font-size:12px;color:#5f6e7d}@media(max-width:640px){body{margin:16px}.meta{grid-template-columns:1fr}.result{font-size:24px}}@media print{.print-btn{display:none}body{margin:16mm}}</style></head><body><button class="print-btn" onclick="window.print()">Print / Save PDF</button><h1>VentTools Site Instruction Sheet</h1><p><strong>${esc(man.label)} ${esc(r.product)} — ${esc(r.reference)}</strong></p><div class="meta"><div class="card"><span class="label">Drawing reference</span><br><strong>${esc(ref)}</strong></div><div class="card"><span class="label">Location</span><br><strong>${esc(loc)}</strong></div></div><div class="card result"><span class="label">Structural opening / required aperture</span><br>${esc(r.opening)}</div><div class="card"><span class="label">Nominal damper</span><br><strong>${esc(r.damper)}</strong><br><span class="label">Finished opening</span><br><strong>${esc(r.finishedStage||r.opening)}</strong><br><span class="label">Bottom of structural opening</span><br><strong>${esc(openingBottom)}</strong><br><span class="label">Nominal duct bottom entered</span><br><strong>${esc(datum)} mm AFFL</strong></div><div class="card"><h2>Finished Opening Setup</h2>${diagram}<div class="diagram-note">VentTools schematic based on the selected verified method — not a copied manufacturer drawing and not to scale.</div></div><div class="card"><h2>Builder / Dryliner Requirements</h2><ul>${builderItems.length?builderItems.map(x=>`<li>${esc(x)}</li>`).join(""):"<li>Follow the selected official manufacturer installation drawing.</li>"}</ul></div><div class="card"><h2>HVAC Installer Requirements</h2><ul>${hvacItems.length?hvacItems.map(x=>`<li>${esc(x)}</li>`).join(""):"<li>Follow the selected official manufacturer installation drawing.</li>"}</ul></div><div class="card warning"><strong>Important:</strong> This sheet is an independent site aid. The current BSB manual, tested installation drawing, project fire strategy and approved construction take precedence.</div><footer>Source: ${esc(p.guide)} — ${esc(p.revision)}. Generated by VentTools V6.5 RC9.</footer></body></html>`;
  try{
    w.document.open();
    ...
    try{
      ...
      try{w.close();}catch(closeErr){}
      fdMsg("warn","The site sheet could not be opened on this device.");
    }
  }
}
